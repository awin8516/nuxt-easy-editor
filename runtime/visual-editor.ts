 ...
 * @returns 匹配的源文件路径数组
 */
export function resolveSourceFiles(routename: string, sourceMap: SourceMap): string[] {
 ...
 */
export function initVisualEditor(config: VisualEditorRuntimeConfig) {
  // 获取当前页面路由
  const routename = window.location.pathname || '/'
  
  // 解析源文件列表
  const sourceFiles = resolveSourceFiles(routename, config.sourceMap)
